...
The `Apple silicon preview` job in `.github/workflows/verify.yml` runs on a `macos-14` ARM64 runner on pushes, pull requests, and manual dispatch. Once 0.4.0 is pushed, open the successful run and download **Companion-Studio-0.4.0-macos-arm64-preview**. The artifact contains a DMG, `SHA256SUMS.txt`, and `BUILD-INFO.txt` with the source commit, workflow run, and qualification status. It is retained for 14 days. No website or GitHub Release is published.
...
The prior 0.3.0 Mac artifact predates provider profiles and must not be reused for this preview. Every CI artifact starts with `qualification_status=awaiting_hardware_verification` and is a candidate for designated testers. Preserve its original metadata and DMG unchanged. Copy the [qualification record template](macos-qualification-template.md) into a separate record and bind results to the source commit, workflow run and attempt, artifact ID, and DMG checksum. Only approve sharing beyond designated testers after all workflow jobs and required manual checks pass for those exact bytes. A rebuilt DMG requires a new record, even when its version is unchanged. Upload itself does not enforce this manual sharing gate or restrict artifact access to designated testers.
...
Record the MacBook model, macOS version, source commit, workflow run and attempt, DMG checksum, and result for every manual qualification. One provider smoke test does not qualify all adapters or model presets. Track full live provider qualification separately under the [provider release gates](provider-chat.md#release-gates), and disclose pending coverage alongside an approved hardware preview. Public release would require a separate Developer ID signing/notarization process and release qualification.
