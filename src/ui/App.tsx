...
import { CloudProvider } from '../services/model';
import { classifyRouteRequest, selectRoute } from '../domain/router';
...
export default function App() {
  ...
  async function startSession() {
    ...
    const localAvailable = await new LlamaCppProvider().isAvailable();
    await repository.saveSession({ id: nextSessionId, companionId: companion.id, createdAt: new Date().toISOString(), executionMode: 'local', memoryMode: 'session' });
    await repository.saveMessage(opening);
    setSessionId(nextSessionId); setMessages([opening]); setStatus((current) => ({ ...current, localProviderAvailable: localAvailable })); setScreen('chat');
  }
  ...
  async function sendMessage() {
    ...
    await repository.saveMessage(userMessage); setInput(''); setBusy(true);
    const nextMessages = [...messages, userMessage]; setMessages(nextMessages);
    const local = new LlamaCppProvider();
    const cloudEndpoint = import.meta.env.VITE_CLOUD_ENDPOINT as string | undefined;
    const route = selectRoute(classifyRouteRequest(content), status);
    let provider: ModelProvider = status.localProviderAvailable ? local : new PrototypeProvider();
    if (route === 'permission-required' && cloudEndpoint) {
      setNotice('This request may need current cloud information. It is staying local until you grant one-time permission in Settings.');
    }
    if (route === 'cloud' && cloudEndpoint) provider = new CloudProvider(cloudEndpoint);
    try {
      const generatedContent = await provider.complete(companion, nextMessages);
      ...
      if (provider.id === 'cloud') {
        await repository.appendAudit({ id: id(), type: 'cloud.request.completed', createdAt: new Date().toISOString(), policyVersion: companion.policyVersion, metadata: { companionId: companion.id, scope: 'single-message', endpointOrigin: new URL(cloudEndpoint!).origin } });
        setStatus((current) => ({ ...current, executionMode: 'cloud', cloudPermission: 'none' }));
      } else {
        setStatus((current) => ({ ...current, executionMode: 'local' }));
      }
      const response: Message = { id: id(), sessionId, role: 'assistant', content: contentOut, createdAt: new Date().toISOString(), provider: provider.id };
      await repository.saveMessage(response); setMessages((current) => [...current, response]);
  ...
  async function toggleCloudPermission() {
    const permission = status.cloudPermission === 'none' ? 'once' : 'none';
    setStatus((current) => ({ ...current, cloudPermission: permission }));
    await repository.appendAudit({ id: id(), type: permission === 'once' ? 'cloud.permission.granted' : 'cloud.permission.revoked', createdAt: new Date().toISOString(), policyVersion: companion.policyVersion, metadata: { scope: permission } });
  }
  ...
  return <div className="app">
    <header>
      <button className="brand brand-button" onClick={() => setScreen('home')}><span className="brand-mark">CS</span><span>Companion Studio</span></button>
      <nav aria-label="Primary navigation">
        <button className={`nav-btn ${screen === 'home' ? 'active' : ''}`} onClick={() => setScreen('home')}>Home</button>
        <button className={`nav-btn ${screen === 'library' ? 'active' : ''}`} onClick={() => setScreen('library')}>My Companions</button>
        <button className={`nav-btn ${screen === 'settings' ? 'active' : ''}`} onClick={() => void openSettings()}>Settings</button>
      </nav>
    </header>
    ...
    <div className="status-strip" aria-label="Runtime status">
      <span className="badge local">● {status.executionMode === 'local' ? 'Local mode' : 'Cloud mode'}</span>
    ...
    <main>
      ...
      {screen === 'detail' && <section className="screen active">
        ...
        <div className="toolbar"><div><h2>{companion.name}</h2><p>{companion.purpose}</p></div><button className="primary-btn" onClick={startSession}>Start session</button></div>
      ...
      {screen === 'chat' && <section className="screen active"><div className="card chat-window">
        <div className="chat-header"><div><h2>{companion.name}</h2><small>{status.localProviderAvailable ? 'Local model connected' : 'Prototype fallback — no model connected'}</small></div><button className="ghost-btn" onClick={() => setScreen('summary')}>End session</button></div>
      ...
      {screen === 'creator' && <section className="screen active"><button className="ghost-btn" onClick={() => setScreen('home')}>← Cancel</button><h2>Create a companion</h2><p>Step {creatorStep + 1} of 5</p>{notice && <div className="safety-notice" role="alert">{notice}</div>}
        ...
        {creatorStep === 3 && <div className="card"><h3>Memory and cloud</h3><p>Durable memory requires explicit consent for each write. Cloud processing asks every time and sensitive content remains local.</p></div>}
      ...
      {screen === 'settings' && <section className="screen active"><h2>Privacy and data</h2>{notice && <div className="safety-notice" role="alert">{notice}</div>}<div className="grid two">
        ...
        <div className="card"><h3>Cloud permission</h3><p>Permission is scoped to one eligible request and is currently <strong>{status.cloudPermission}</strong>. Sensitive requests remain local.</p><button className="secondary-btn" onClick={() => void toggleCloudPermission()}>{status.cloudPermission === 'none' ? 'Allow once' : 'Revoke'}</button></div>
    ...
    <footer>Prototype v0.2 · Local-first AI with enforceable boundaries</footer>
  </div>;
}
